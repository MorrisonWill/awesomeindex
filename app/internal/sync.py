...
from app.models import Repository, Project, RepositoryCreate, ProjectCreate
...
class SyncService:
    ...
    async def discover_awesome_repositories(self, limit: int = 50) -> List[dict]:
        ...
        for repo in repos:
            ...
            if (name.startswith("awesome-") or 
                "awesome" in description and 
                ("list" in description or "collection" in description)):
    ...
    def _upsert_repository(self, session: Session, repo_data: dict) -> Repository:
        ...
        if existing_repo:
            ...
            existing_repo.homepage_url = repo_data.get("homepage")
            existing_repo.stars = repo_data.get("stargazers_count")
            existing_repo.forks = repo_data.get("forks_count")
            ...
            new_repo = Repository.model_validate(repo_create.model_dump())
    ...
    async def _sync_repository_projects(
        ...
        for parsed_project in parsed_projects:
            project_create = ProjectCreate(
                ...
                raw_markdown=parsed_project.raw_markdown
            )
            project = Project.model_validate(project_create.model_dump())
            session.add(project)
    ...
    async def _index_projects_for_search(self, projects: List[Project]):
        ...
        for project in projects:
            doc = {
                ...
                "repository_name": project.repository.name if project.repository else None
            }
            ...
            try:
                ...
                if repository:
                    ...
                    results["repositories"].append({
                        ...
                        "status": "synced",
                        "projects_count": len(repository.projects) if repository.projects else 0
                    })
                else:
                    results["errors"] += 1
                    results["repositories"].append({
